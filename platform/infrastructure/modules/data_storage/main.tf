...
resource "aws_s3_bucket" "raw_data" {
  ...
  tags = merge(
    var.tags,
    {
      ...
      Purpose     = "Raw data storage for Black Alder Platform"
    }
  )
}
...
resource "google_storage_bucket" "raw_data" {
  ...
  labels = merge(
    ...
    {
      environment = var.environment
      purpose     = "raw-data-storage-black-alder-platform"
    }
  )
}
...
resource "google_bigquery_dataset" "default" {
  ...
  project                     = var.gcp_project_id
  location                    = var.gcp_region
  description                 = "Dataset for Black Alder Platform processed data"
  ...
  labels = merge(
    ...
    {
      environment = var.environment
      purpose     = "processed-data-storage-black-alder-platform"
    }
  )
}
